...
**Runtime CSP-violation observability: gap.** The perf-harness has no `securitypolicyviolation` listener, and `packages/perf-harness/acceptance.ts` only substring-checks that the CSP header is present — not that it's enforced cleanly. What we _do_ have evidence for: (a) the header is asserted present on every n=10 acceptance run, and (b) the directive list is structurally type-locked (see `_CspDirectivePrefixIsExact` above) so silent widening is impossible. A runtime violation collector should land before M-13 if the audit needs the stronger "no violations observed" claim. Recorded as follow-up.
