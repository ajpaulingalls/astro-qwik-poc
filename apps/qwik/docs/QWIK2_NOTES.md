...
5. **Leaf-component convention** (convention defined at _M3 scaffolding § Leaf component convention_; conversion outcome at _sprint-009 § What landed (this session)_ item 2 — flagged in sprint-008 audit `## sprint-008 — story-005 — Article JS budget audit`, executed in sprint-009). Stateless leaves (no signals, no `$()` handlers, no `Slot`) use plain functions, not `component$`, to avoid the per-call-site QRL serialization boundary + chunk overhead. The conversions of HeroCard / MostPopular / CuratedCollection / Footer landed −558 bytes / −4 chunks. **Exception**: leaves rendered inside reactive `signal.value.map(...)` (StoryCard via LoadMoreButton) keep `component$` because plain-function leaves break the reactive append.
...
4. **lhPerf gate split per-target** (`packages/perf-harness/cli_helpers.ts`): added `QWIK_LH_PERF_FLOOR`. Qwik 2 beta.32 LH-throttled measures 81-90 (range re-anchored at sprint-009 capstone — qwik/index 5-run median dropped from 85 to 81 between sprint-008 and sprint-009 closes despite no homepage code changes, attributable to framework-runtime drift in the beta line). The prior shared `STRETCH_CWV.lhPerf = 98` was hard-failing every Qwik perf-harness run since sprint-006 — silent gate noise, not signal. Astro pages still hold the stretch 98; Qwik pages override to the measured-realistic 80 floor (initially set to 85 at sprint-009 split, lowered to 80 at sprint-009 capstone close). Re-evaluate when Qwik 2 stable ships.
...
2. **Try the 5 additional leaf candidates** flagged by the simplify code-reuse agent: `HeroCard`, `MostPopular`, `StoryCard`, `CuratedCollection`, `Footer`. These appear multiple times per homepage render so the inline-vs-chunk math is more favorable than for the embeds. The disqualified candidates (`SectionHeading` uses `Slot`, `VerticalVideoCarousel` has `onClick$`) stay as `component$`. — _Outcome 2026-04-28: 4 of 5 converted (HeroCard/MostPopular/CuratedCollection/Footer) for -558 bytes / -4 chunks. StoryCard kept as `component$` because LoadMoreButton renders it inside `posts.value.map(...)` over a mutable signal — the reactive append needs the QRL boundary. Convention applies cleanly to leaves rendered over `routeLoader$` data (CuratedCollection at routes/index.tsx:85) but not to leaves inside reactive-signal maps. See StoryCard.tsx top comment._
