...
**Reports are gitignored and untracked by default.** The package-level `packages/perf-harness/.gitignore` excludes `reports/*.json` and `reports/*.md` (RUN_NOTES.md is carved out via `!reports/RUN_NOTES.md` because it's methodology, not regenerated output). Reports are **re-added with `git add -f` only at milestone-validation boundaries** (e.g., M-3 perf-harness landing, M-9 CWV gate, M-13 final comparison) where a published numbers snapshot needs to live in git as a deliverable artifact. Between milestones, harness re-runs regenerate these files locally but leave the working tree clean — they no longer pollute `git status` during unrelated feature commits. The canonical M-13 n=10 reports below were force-added in commit `fc5b41d`; future milestone snapshots use the same `git add -f packages/perf-harness/reports/<files>` pattern. Non-canonical artifacts (`*.baseline.json`, `*.lever1.json` from sprint-007/008) stay untracked.
